...
describe('WebViewProvider.attachToView', () => {
  ...
  it('surfaces insight slash-command errors as a failed task state', async () => {
    const { postMessage } = await setupAttachedProvider();
    ...
    slashCommandNotificationCallbackRef.current?.({
      ...
      command: '/insight',
      messageType: 'error',
      message: 'Insight generation failed due to invalid session data.',
    });

    expect(postMessage).toHaveBeenCalledWith({
      type: 'insightFailed',
      data: {
        error: 'Insight generation failed due to invalid session data.',
      },
    });
    expect(postMessage).not.toHaveBeenCalledWith({
      type: 'streamChunk',
      data: {
        chunk: 'Insight generation failed due to invalid session data.\n',
      },
    });
  });
